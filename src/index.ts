...
export function tokenize(contents: string): string[] {
	...
	tokenizedContents.forEach(([token, content]) => {
		...
		switch (token) {
			...
			case 'exit':
				...
				break;
			case 'pause':
				// I haven't found a way to properly pause execution in Node.js like in Batch scripts, so just sleep for 3 seconds
				compiledContents += `setTimeout(() => {}, 3000);\n`;
				break;
			case 'set':
				// Set contentNoQuotes to the content without double quotes
				let contentNoQuotes = content.replace(/"/g, '');
				// Split the contentNoQuotes with '=' and set the first part as the variable name and the second part as the variable value

				let [variableName, variableValue] = contentNoQuotes.split('=');
				...
				// Check if the compiled contents already contain "let variableName"
				// If it does, do "variableName = variableValue" instead of "let variableName = variableValue"

				if (compiledContents.includes(`let ${variableName}`)) {
					compiledContents += `${variableName} = ${variableValue};\n`;
				} else {
					compiledContents += `let ${variableName} = ${variableValue};\n`;
				}
				break;
			case '::':
				// Compile comment
				compiledContents += `// ${content}\n`;
				break;
			case 'rem':
				// Compile comment
				compiledContents += `// ${content}\n`;
		...
		compiledContents = compiledContents.replace(/%([^%]+)%/g, '${$1}');
	});
